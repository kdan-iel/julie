...
import { Check, Heart, Send, Sparkles, User, Mail, Users, Utensils, Music, MessageSquare } from 'lucide-react';
...
export function RSVPSection() {
  ...
  return (
    <section id="rsvp" className="py-24 px-6 relative bg-[#4B5842] text-[#FBF8F3] overflow-hidden">
          ...
          <AnimatePresence mode="wait">
            {!isSubmitted ? (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                  ...
                  >
                    ...
                    <div className="w-5 h-5 rounded-full border-2 border-[#A4193D] flex items-center justify-center">
                      {formData.attending === 'yes' && <div className="w-2.5 h-2.5 rounded-full bg-[#A4193D]" />}
                    </div>
                  ...
                  >
                    ...
                    <div className="w-5 h-5 rounded-full border-2 border-white/40 flex items-center justify-center">
                      {formData.attending === 'no' && <div className="w-2.5 h-2.5 rounded-full bg-red-400" />}
                    </div>
                ...
                >
                  {isSubmitting ? (
                    <span>{weddingContent.rsvp.fields.sendingBtn}</span>
                  ) : (
                    <>
                      <Send className="w-4 h-4" />
                      <span>{weddingContent.rsvp.fields.submitBtn}</span>
                    </>
                  )}
                </button>
              </motion.form>
              ...
              >
                ...
                <div className="p-4 rounded-xl bg-white/5 border border-white/10 text-left text-xs space-y-2 max-w-sm mx-auto">
                  <p><span className="text-[#A4193D]">Nom :</span> {formData.firstName} {formData.lastName}</p>
                  <p><span className="text-[#A4193D]">Présence :</span> {formData.attending === 'yes' ? 'Présent(e)' : 'Absent(e)'}</p>
                  {formData.attending === 'yes' && (
                    <>
                      <p><span className="text-[#A4193D]">Invités :</span> {formData.guestCount}</p>
                      <p><span className="text-[#A4193D]">Menu :</span> {formData.menuChoice}</p>
                    </>
                  )}
                </div>
